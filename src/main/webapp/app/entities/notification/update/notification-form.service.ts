...
type NotificationFormDefaults = Pick<NewNotification, 'id' | 'isNotified'>;

type NotificationFormGroupContent = {
  id: FormControl<INotification['id'] | NewNotification['id']>;
  minutes: FormControl<INotification['minutes']>;
  isNotified: FormControl<INotification['isNotified']>;
  userHasMatch: FormControl<INotification['userHasMatch']>;
};
...
export class NotificationFormService {
  createNotificationFormGroup(notification: NotificationFormGroupInput = { id: null }): NotificationFormGroup {
    ...
    return new FormGroup<NotificationFormGroupContent>({
      ...
      minutes: new FormControl(notificationRawValue.minutes),
      isNotified: new FormControl(notificationRawValue.isNotified),
      userHasMatch: new FormControl(notificationRawValue.userHasMatch),
    });
  }
