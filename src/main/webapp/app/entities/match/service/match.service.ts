...
type RestOf<T extends IMatch | NewMatch> = Omit<T, 'startingDate' | 'endingDate'> & {
...
export class MatchService {
  ...
  protected convertDateFromClient<T extends IMatch | NewMatch | PartialUpdateMatch>(match: T): RestOf<T> {
    return {
      ...match,
      startingDate: match.startingDate?.format(DATE_FORMAT) ?? null,
      endingDate: match.endingDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMatch: RestMatch): IMatch {
    return {
      ...restMatch,
      startingDate: restMatch.startingDate ? dayjs(restMatch.startingDate) : undefined,
      endingDate: restMatch.endingDate ? dayjs(restMatch.endingDate) : undefined,
    };
  }
